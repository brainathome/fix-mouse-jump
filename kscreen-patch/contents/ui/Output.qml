...
Item {
    ...
    // Expose model data for Screen.qml's position correction
    readonly property point modelPosition: model.position ? model.position : Qt.point(0, 0)
    readonly property size modelSize: model.size ? model.size : Qt.size(1, 1)

    // Use corrected position if available (gap-free, bottom-aligned)
    readonly property point displayPos: {
        var cp = screen.correctedPositions;
        if (cp && cp[model.index] !== undefined) {
            return cp[model.index];
        }
    ...
    x: displayPos.x / screen.relativeFactor + screen.xOffset
    y: displayPos.y / screen.relativeFactor + screen.yOffset
